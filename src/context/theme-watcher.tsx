...
import { defaultThemeId } from '@/config/themes'; // Ensure defaultTheme is imported
...
export const ThemeWatcher: React.FC<ThemeWatcherProps> = ({ children }) => {
  ...
  useEffect(() => {
    const themeId = context?.state?.theme || defaultThemeId; // Fallback to default
    ...
    // Ensure dark class is present (if your themes are designed for dark mode)
