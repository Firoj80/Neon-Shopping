...
export const ItemCard: React.FC<ItemCardProps> = ({ item, onEdit, onDelete }) => {
  ...
  return (
    <Card className={cn(
        "rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 flex items-center p-3 w-full border-primary/20 hover:border-primary/50",
        item.checked ? "bg-card/60 border-border/10" : "bg-card" // Dim checked items slightly
     )}>
        <Checkbox
            id={`item-${item.id}`}
            checked={item.checked}
            onCheckedChange={handleToggleCheck}
            className="mr-3 shrink-0 border-primary data-[state=checked]:bg-primary data-[state=checked]:text-primary-foreground data-[state=checked]:border-primary"
        ...
        <div className="flex-grow min-w-0">
            <label
                htmlFor={`item-${item.id}`}
                 className={cn(
                "text-sm font-medium leading-tight cursor-pointer block truncate", // Use truncate for long names
        ...
        <div className="flex space-x-1 ml-auto shrink-0 pl-2">
            <Button
            variant="ghost"
            size="icon"
