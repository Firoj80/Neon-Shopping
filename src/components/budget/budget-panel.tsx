...
export const BudgetCard: React.FC = () => {
  ...
  return (
    <Card className="w-full bg-card border-primary/30 shadow-neon glow-border mb-4 sm:mb-6">
       <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-1 sm:space-y-0 pb-2 pt-3 px-4 sm:pb-3 sm:pt-4 sm:px-6">
        <div className="flex items-center gap-2">
             <Wallet className="h-5 w-5 text-primary shrink-0" />
             <div className="flex flex-col">
                <CardTitle className="text-base sm:text-lg font-semibold text-primary leading-tight">
                    List Budget: <span className="text-secondary">{selectedList.name}</span>
                </CardTitle>
        ...
        <Dialog open={isEditingBudget} onOpenChange={setIsEditingBudget}>
          <DialogTrigger asChild>
              <Button variant="ghost" size="sm" className="text-primary/80 hover:text-primary hover:bg-primary/10 mt-2 sm:mt-0 self-start sm:self-center glow-border-inner">
       ...
       <CardContent className="space-y-3 sm:space-y-4 px-4 pb-4 sm:px-6 sm:pb-5 glow-border-inner">
         <div className="space-y-1">
            ...
            <Progress
                ...
                className={cn(
                    "h-2 sm:h-3 bg-secondary/20 glow-border-inner",
         ...
         <div className="flex items-center justify-between pt-1 sm:pt-2">
            <div className="flex items-center gap-2 text-sm sm:text-lg font-medium">
                 <Coins className={cn("h-4 w-4 sm:h-5 sm:w-5", isOverBudget ? 'text-destructive' : 'text-primary')} />
                <span className={cn("text-neonText")}>
                    Remaining:
                </span>
            </div>
             <div className={cn("text-base sm:text-xl font-bold", isOverBudget ? 'text-destructive' : 'text-primary')}>
...
const BudgetCardSkeleton: React.FC<{ selectedListName?: string }> = ({ selectedListName }) => {
  return (
    <Card className="w-full bg-card border-border/20 shadow-md animate-pulse mb-4 sm:mb-6 glow-border">
       <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-1 sm:space-y-0 pb-2 pt-3 px-4 sm:pb-3 sm:pt-4 sm:px-6">
        ...
        <Skeleton className="h-8 w-24 rounded-md self-start sm:self-center" />
      </CardHeader>
       <CardContent className="space-y-3 sm:space-y-4 px-4 pb-4 sm:px-6 sm:pb-5 glow-border-inner">
        <div className="space-y-1">
          ...
          <Skeleton className="h-2 sm:h-3 w-full rounded-full glow-border-inner" />
