  ...
  useEffect(() => {
   ...
   useEffect(() => {
       if (budget.lastSetDate) {
           try {
             setBudgetDateLabel(format(new Date(budget.lastSetDate + 'T00:00:00Z'), 'MMM d, yyyy')); // Added Z for UTC
           } catch (e) {
        ...
        <div className="flex items-center gap-2">
             ...
             <TooltipProvider>
                <Tooltip delayDuration={100}>
                    ...
                    <TooltipContent side="bottom" className="text-xs bg-popover text-popover-foreground border-primary/30 glow-border-inner">
       ...
       <CardContent className="space-y-3 sm:space-y-4 px-4 pb-4 sm:px-6 sm:pb-5">
         ...
         <div className="flex items-center justify-between pt-1 sm:pt-2">
            <div className="flex items-center gap-2 text-sm sm:text-lg font-medium">
                ...
                <span className={cn("text-neonText")}> {/* Use neonText for the label */}
                    Remaining Today:
                </span>
            </div>
             <div className={cn("text-base sm:text-xl font-bold", isOverBudget ? 'text-destructive' : 'text-primary')}> {/* Use theme colors for value */}
