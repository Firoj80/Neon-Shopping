...
import { showPreparedInterstitialAd } from '@/components/admob/ad-initializer'; // Import the AdMob interstitial function
import dynamic from 'next/dynamic';
import ClientOnly from '@/components/client-only';


// Dynamically import AdComponent for the banner ad, ensuring it only runs client-side
const AdComponent = dynamic(() => import('@/components/admob/ad-component').then(mod => mod.AdComponent), {
    ...
    loading: () => <div className="fixed bottom-0 left-0 right-0 h-[50px] bg-muted/10 flex items-center justify-center text-xs text-muted-foreground z-40 backdrop-blur-sm">Loading Ad...</div>
});
...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuItems = [
    ...
    { href: '/settings', label: 'Settings', icon: Settings, triggerAd: true },
  ...
  const handleLinkClick = async (item: { href: string; triggerAd: boolean }, event: React.MouseEvent<HTMLAnchorElement, MouseEvent>) => {
     ...
     // Show interstitial ad if configured for this link
     if (item.triggerAd) {
       try {
         await showPreparedInterstitialAd(); // Attempt to show the ad
         // You might want a small delay here before navigating,
         // although AdMob often handles this timing.
  ...
  return (
    <>
       {/* Fixed AdMob Banner Placeholder at the bottom */}
       <ClientOnly>
            <AdComponent />
       </ClientOnly>
      ...
      <SidebarInset className="flex flex-col min-h-screen">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 pb-[60px] md:pb-8"> {/* Increased bottom padding for banner */}
          {children}
        </main>
      </SidebarInset>
    </>
