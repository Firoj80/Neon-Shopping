...
import {
  ...
  SelectLabel,
  SelectValue
} from '@/components/ui/select'; // Ensure Select components are imported from here
...
const formSchema = z.object({
  ...
  price: z.preprocess( // Allow empty string for price, defaulting to 0 or null
...
export const AddEditItemModal: React.FC<AddEditItemModalProps> = ({ isOpen, onClose, itemData, currentListId }) => {
  ...
  const { register, handleSubmit, control, reset, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: {
      name: '',
      quantity: 1,
      price: null, // Default to null for placeholder
      category: '',
    }
  });
    ...
    if (isOpen) {
      if (itemData) {
        reset({
          ...
          price: itemData.price === 0 ? null : itemData.price, // Show placeholder if 0
  ...
  const onSubmit = async (data: FormData) => {
    if (!currentListId || !currentUserId) { // Use currentUserId from appState
    ...
    const payload = {
      ...data,
      id: itemData ? itemData.id : uuidv4(), // Generate new ID if not editing
      listId: currentListId,
      userId: currentUserId, // Use anonymous userId from context
      price: data.price ?? 0, // Ensure price is a number, default to 0 if null
      checked: itemData ? itemData.checked : false, // Keep checked state if editing
      dateAdded: itemData ? itemData.dateAdded : Date.now(), // Keep dateAdded if editing
    };

    try {
      if (itemData) {
        dispatch({ type: 'UPDATE_SHOPPING_ITEM', payload: payload as ShoppingListItem });
        toast({ title: "Success", description: "Item updated." });
      } else {
        dispatch({ type: 'ADD_SHOPPING_ITEM', payload: payload as ShoppingListItem });
                ...
                render={({ field }) => (
                  <Input
                    ...
                    onChange={(e) => {
                     ...
                     value={field.value === 0 ? '' : String(field.value)} // Show empty if 0
                    placeholder="1"
                    className="border-primary/50 focus:border-primary focus:shadow-neon focus:ring-primary text-sm glow-border-inner"
                    min="1"
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Input
                    ...
                    value={field.value === null || field.value === undefined ? '' : String(field.value)} // Keep "" for null/undefined
                    placeholder="0.00"
                    className="border-primary/50 focus:border-primary focus:shadow-neon focus:ring-primary text-sm glow-border-inner"
                    min="0"
            ...
            <Controller
              ...
              render={({ field }) => (
                <Select
                  ...
                  value={field.value || 'uncategorized'} // Ensure a valid default if field.value is empty
                  disabled={categories.filter(cat => cat.id !== 'uncategorized' && cat.userId === currentUserId).length === 0 && field.value === 'uncategorized' && !categories.some(c=>c.id === 'uncategorized' && c.userId === null) }
                >
                  <SelectTrigger
                    id="category"
                  ...
                  >
                    <ScrollArea className="h-[200px] w-full">
                      <SelectGroup>
                        ...
                        {categories.filter(cat => cat.userId === null || cat.userId === currentUserId).map((category) => (
                          <SelectItem
                            key={category.id}
                            value={category.id}
                            className={cn(
                            ...
                            disabled={category.id === 'uncategorized' && categories.filter(c => c.userId === currentUserId).length === 0 && category.userId === null}
                          >
                            {category.name}
                          </SelectItem>
                        ))}
                         {categories.filter(cat => cat.userId === null || cat.userId === currentUserId).length === 0 && (
