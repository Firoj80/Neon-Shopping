...
try {
    ...
    $stmt_categories = $conn->prepare("SELECT id, user_id, name FROM categories WHERE user_id = ? OR user_id IS NULL");
    $stmt_categories->execute([$user_id]);
    while ($row = $stmt_categories->fetch(PDO::FETCH_ASSOC)) {
        // userId will be NULL for global categories, or the user's ID for their custom ones
        $response_data['categories'][] = $row;
    }
    
    // Fetch User Preferences (currency, premium status from users table)
    ...
    if ($prefs_currency = $stmt_prefs_currency->fetch(PDO::FETCH_ASSOC)) {
        if (!empty($prefs_currency['currency_code'])) {
            ...
            $response_data['user_preferences']['currency'] = ['code' => $prefs_currency['currency_code'], 'symbol' => '$', 'name' => $prefs_currency['currency_code']]; // Placeholder
        }
    }
