...
try {
    ...
    if ($user) {
        $is_premium = false;
        $is_expired = false; // Flag to check if a premium subscription has expired
        ...
        if ($user['subscription_status'] === 'premium') {
            if ($user['subscription_expiry_date'] === null) { // Lifetime or non-expiring premium
                ...
                if ($expiry_timestamp && $expiry_timestamp > time()) {
                    ...
                    // Premium subscription exists but has expired
                    $is_expired = true; 
                }
            }
        }
        
        // If subscription was premium but now detected as expired, downgrade the user
        if ($is_expired) {
            ...
            // $is_premium is already false due to expiry check
        }


        send_json_response([
            'success' => true,
            'isPremium' => $is_premium, // This will be false if expired
            'status' => $is_premium ? 'premium' : 'free', // Current effective status
            'expiryDate' => ($is_premium && $user['subscription_expiry_date']) ? $user['subscription_expiry_date'] : null
        ]);
    } else {
