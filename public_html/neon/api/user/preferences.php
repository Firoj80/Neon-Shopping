...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    try {
        ...
        if ($prefs && !empty($prefs['currency_code'])) {
            ...
            // Ideally, fetch full currency details (symbol, name) from a 'currencies' table if you have one.
            send_json_response(['success' => true, 'currency' => ['code' => $prefs['currency_code'], 'symbol' => '$', 'name' => $prefs['currency_code']]]); // Placeholder symbol/name
        } else {
            // No preference found, client should use its default or auto-detected one
    ...
    // Optional: Validate currency_code against a list of supported codes.
    ...
    try {
        ...
        send_json_response(['success' => true, 'message' => 'Currency preference updated.', 'currency' => ['code' => $currency_code, 'symbol' => '$', 'name' => $currency_code]]); // Placeholder
    } catch (PDOException $e) {
