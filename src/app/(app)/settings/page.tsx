...
import { Banknote, Layers, Trash2, Edit, PlusCircle, Save } from 'lucide-react';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const fetchCurrencies = async () => {
      ...
      try {
        ...
        // Auto-detect currency if none is set in context/localStorage
        if (!state.currency.code) { // Assuming empty code means not set
          handleAutoDetectCurrency();
        }
      } catch (error) {
  ...
  const handleAutoDetectCurrency = async () => {
    ...
    try {
      const detectedCurrency = await getUserCurrency(); // Fetch based on location
      if (detectedCurrency) {
        handleSelectCurrency(detectedCurrency);
        toast({
          title: "Currency Detected",
          description: `Currency set to ${detectedCurrency.name} (${detectedCurrency.code}) based on your location.`,
          variant: "default",
        });
      } else {
         toast({
            title: "Detection Failed",
            description: "Could not automatically detect your currency. Please select one manually.",
            variant: "destructive",
          });
      }
    } catch (error) {
      ...
      toast({
        ...
        description: "An error occurred while detecting your currency.",
        variant: "destructive",
      });
    } finally {
        ...
        <CardContent className="space-y-4">
          ...
          <form onSubmit={handleAddCategory} className="flex items-end gap-2">
           ...
           <div className="space-y-2 pt-4">
             ...
             {categories.length > 0 ? (
                <ul className="border border-border/30 rounded-md divide-y divide-border/30">
...
const SettingsPageSkeleton: React.FC = () => (
  <div className="space-y-6 animate-pulse">
    ...
    <Card className="bg-card border-border/20 shadow-md">
      ...
      <CardContent className="space-y-4">
         <Skeleton className="h-10 w-full sm:w-1/3 rounded-md" /> {/* Auto-Detect Button */}
         <div className="grid gap-2">
