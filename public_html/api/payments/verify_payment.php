...
try {
    ...
    // Determine expiry date based on plan_id (these plan_ids should match IDs in your 'premium_plans' table)
    if ($plan_id === 'monthly_basic') { // Match this with your premium_plans table ID
        $current_date->modify('+1 month');
    } elseif ($plan_id === 'three_month_standard') { // Match this with your premium_plans table ID
        $current_date->modify('+3 months');
    } elseif ($plan_id === 'yearly_premium') { // Match this with your premium_plans table ID
        $current_date->modify('+1 year');
    } else {
        send_json_response(['success' => false, 'message' => 'Invalid plan ID for subscription duration.'], 400);
    }
        ...
        $_SESSION['subscription_expiry_date'] = $expiry_date_sql;
         if (isset($_SESSION['user_data'])) { // If you store user data object in session
            $_SESSION['user_data']['isPremium'] = true; 
             $_SESSION['user_data']['subscriptionStatus'] = $new_status;
             $_SESSION['user_data']['subscriptionExpiryDate'] = $expiry_date_sql;
        }
